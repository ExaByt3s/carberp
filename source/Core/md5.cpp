...
void MD5Update (MD5_CTX *context, unsigned char *input, unsigned int inputLen)
{
	  ...
	  if (inputLen >= partLen) 
	  {
		 m_memcpy ((POINTER)&context->buffer[index], (POINTER)input, partLen);
	  ...
	  m_memcpy ((POINTER)&context->buffer[index],
	              (POINTER)&input[i],
		      inputLen-i);
}
...
void MD5Final (unsigned char digest[16], MD5_CTX *context)
{
	 ...
	 */
	m_memset ((POINTER)context, 0, sizeof (*context));
}
