...
{
	// Функция возвращает истину если строка содержит переводы в
	// стиле линукс \n
	...
	// Передираем строку в поисках символа \r
    BYTE Count = 0;
	for (; *S != 0; S++)
	{
		...
		if (*S == '\n')
		{
			// Подсчитываем количество поподаний символа \n
			// 3 вхождения считаем достаточным для надёжности
			Count++;
			if (Count >= 3)
				break;
		}
...
{
	// Функция приводит строку к линукс формату - перевод строк
	// только символ \n
	if (STR::IsEmpty(S))
		return NULL;
